...
public class BLECommand extends Command {
	...
	public void preparePayload(Types.Commands paramCommands,
		...
		if (str != null) {
			if (paramList.size() > 0) {
				Logs.log(
						Logs.LogTypes.codeerror,
						"Caused by: Wrong use of the preparePayload function. BLE Commands do not have parameters");
				payload = "";
				return;
			}
			...
			Logs.log(Logs.LogTypes.codeerror,
					"Caused by: Terminator not Existing or null command");
			payload = "";
		}
	}

	public static void setCommand(Types.Commands paramCommands,
		...
		if ((paramCommands != null) && (paramString1 != null)) {
			try {
				...
				if (i != 0) {
					a.put(paramCommands, new Command.a(paramString1,
							paramString2, paramString3));
				} else {
					Logs.log(
							Logs.LogTypes.codeerror,
							" Caused by Value entry:  wrong format.  "
									+ paramCommands
									+ " - "
									+ "Please validate the corresponding JSON file.");
				...
				Logs.log(
						Logs.LogTypes.codeerror,
						" Verify if the corresponding enum entry in Types exists.  Caused by Value entry: "
								+ paramCommands
								+ " - "
								+ "Please validate the corresponding JSON file.");
				throw new IllegalArgumentCheckedException(
						" Caused by Value entry:  Verify if the corresponding enum entry in Types exists. "
								+ paramString1 + paramCommands, localException);
			}
		} else {
			throw new IllegalArgumentCheckedException(
					" Caused by Value entry:   Wrong parameters sent to the function: Key and/or Value are null ");
		}
	}
	...
	public static String getCommand(Types.Commands paramCommands) {
		try {
			...
			Logs.log(
					Logs.LogTypes.codeerror,
					" Caused by Value entry:   Wrong parameters sent to the function:Command or CommandType are null");
			return null;
		} catch (Exception localException) {
			Logs.log(Logs.LogTypes.codeerror,
					" Caused by Value entry:  Command not found. "
							+ paramCommands.toString());
		}
		return null;
	}
