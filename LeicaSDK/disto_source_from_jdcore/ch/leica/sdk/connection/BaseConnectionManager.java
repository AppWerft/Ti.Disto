...
public abstract class BaseConnectionManager {
	...
	public BluetoothDevice getBluetoothDevice() {
		...
		Logs.log(Logs.LogTypes.codeerror,
				"NULL Value - Connection Parameter BlueToothDevice was not set");
		return null;
	}
	...
	public void pauseBTConnection(
			Device.BTConnectionCallback paramBTConnectionCallback) {
	}
		...
		for (;;) {
			if (a == true) {
				Logs.log(Logs.LogTypes.debug,
						"connection already killed. return.");
			} else {
				try {
					Thread.sleep(100L);
				} catch (InterruptedException localInterruptedException) {
					Logs.log(Logs.LogTypes.verbose,
							"sleep thread interrupted. NF");
				}
				try {
					...
					try {
						if (localInputStream != null) {
							...
							if (Build.VERSION.SDK_INT < 21) {
								if (!socket.isClosed()) {
									...
									while (!localInputStreamReader.ready()) {
										((WaitAmoment) localObject2)
												.waitAmoment(500L);
									}
									Logs.log(Logs.LogTypes.verbose,
											"inputStreamReader is ready!");
									int i = localInputStream.available();
									Logs.log(Logs.LogTypes.verbose,
											"inputStream.available: " + i);
									char[] arrayOfChar = new char[i];
									int j = localInputStreamReader
											.read(arrayOfChar);
									Logs.log(Logs.LogTypes.verbose,
											"readCount: " + j);
									...
									try {
										localReceivedData
												.parseReceivedWifiData(str);
									} catch (DeviceException localDeviceException2) {
										localErrorObject = new ErrorObject(
												2702,
												localDeviceException2
														.getMessage());
									}
									if (wifiReceivedDataListener != null) {
										wifiReceivedDataListener
												.onResponseReceived(
														localReceivedData,
														localErrorObject);
									} else {
										Logs.log(Logs.LogTypes.debug,
												"no receivedData listener is set");
									}
								}
							} else if (!socket.isClosed()) {
								if (localBufferedReader == null) {
									localBufferedReader = new BufferedReader(
											localInputStreamReader, 128000);
								}
								localObject1 = localBufferedReader.readLine();
								if (localObject1 == null) {
									Logs.log(Logs.LogTypes.codeerror,
											"receivedData is null. sth is wrong. continue the loop.");
									if (b > 0) {
								...
								try {
									localReceivedData
											.parseReceivedWifiData((String) localObject1);
								} catch (DeviceException localDeviceException1) {
									localObject2 = new ErrorObject(2702,
											localDeviceException1.getMessage());
								}
								if (wifiReceivedDataListener != null) {
									wifiReceivedDataListener
											.onResponseReceived(
													localReceivedData,
													(ErrorObject) localObject2);
								} else {
									Logs.log(Logs.LogTypes.debug,
											"no receivedData listener is set");
								}
							...
							Logs.log(Logs.LogTypes.codeerror,
									"inputStream  = NULL, Validate the response channel code");
						}
					} catch (Exception localException) {
						Logs.log(Logs.LogTypes.exception, "Error Caused by: ",
								localException);
						if (wifiReceivedDataListener != null) {
							wifiReceivedDataListener.onResponseReceived(
									null,
									new ErrorObject(3304, localException
											.getMessage()));
						} else {
							Logs.log(Logs.LogTypes.debug,
									"no receivedData listener is set");
					...
					Logs.log(Logs.LogTypes.exception, "IOException"
							+ localIOException2.getMessage());
					if (wifiReceivedDataListener != null) {
						wifiReceivedDataListener.onResponseReceived(
								null,
								new ErrorObject(3302, localIOException2
										.getMessage()));
					} else {
						Logs.log(Logs.LogTypes.debug,
								"no receivedData listener is set");
			...
			if (a == true) {
				...
				try {
					...
					if ((eventSocket == null) || (!eventSocket.isConnected())) {
						...
						Logs.log(Logs.LogTypes.debug,
								"connect event socket successful");
					}
					try {
						if (localInputStream != null) {
							localObject = "";
							...
							if (Build.VERSION.SDK_INT < 21) {
								if (!eventSocket.isClosed()) {
									...
									if (wifiReceivedDataListener != null) {
										wifiReceivedDataListener
												.onEventDataReceived(localReceivedData);
									} else {
										Logs.log(Logs.LogTypes.debug,
												"no receivedData listener is set");
									}
								...
								if (!eventSocket.isClosed()) {
									Logs.log(Logs.LogTypes.verbose,
											"event socket is not closed");
									Logs.log(Logs.LogTypes.verbose,
											"now waiting for receiving data");
									...
									localReceivedData
											.parseReceivedWifiData((String) localObject);
									if (wifiReceivedDataListener != null) {
										wifiReceivedDataListener
												.onEventDataReceived(localReceivedData);
									} else {
						...
						Logs.log(Logs.LogTypes.exception, "device exception: "
								+ localDeviceException.getMessage());
					}
				} catch (IOException localIOException) {
					Logs.log(Logs.LogTypes.exception, "IOException"
							+ localIOException.getMessage(), localIOException);
					ErrorObject.sendErrorEventChannelSocketNotConnecting("",
							errorListener, this);
					setState(ConnectionState.disconnected, true);
					break;
				}
	...
	public void foundAvailableBluetoothDevice(String paramString,
			BluetoothDevice paramBluetoothDevice, boolean paramBoolean1,
			boolean paramBoolean2) {
		if (scanDevicesListener != null) {
			Logs.log(Logs.LogTypes.debug,
					"available BLE device found, tell listener now");
			scanDevicesListener.onBluetoothDeviceFound(paramString,
					paramBluetoothDevice, paramBoolean1, paramBoolean2);
		} else {
			Logs.log(Logs.LogTypes.debug,
					"available BLE device found, but no listener is set");
		}
	}
	...
	public void disconnectedACLBluetoothDevice(String paramString) {
		if (scanDevicesListener != null) {
			Logs.log(Logs.LogTypes.debug,
					"BLE device disconnected in settings, tell listener now");
	...
	public void foundAvailableHotspotDevice(String paramString) {
		if (scanDevicesListener != null) {
			Logs.log(Logs.LogTypes.debug,
					"available HOTSPOT device found, tell listener now");
			scanDevicesListener.onHotspotDeviceFound(paramString);
		} else {
			Logs.log(Logs.LogTypes.debug,
					"available HOTSPOT device found, but no listener is set");
		}
	}
	...
	public void foundAvailableApDevice(String paramString1, String paramString2) {
		if (scanDevicesListener != null) {
			Logs.log(Logs.LogTypes.debug,
					"available AP device found, tell listener now");
			scanDevicesListener.onApDeviceFound(paramString1, paramString2);
		} else {
			Logs.log(Logs.LogTypes.debug,
					"available AP device found, but no listener is set");
		}
	}

	public void foundAvailableRndisDevice(String paramString1,
			String paramString2) {
		if (scanDevicesListener != null) {
			Logs.log(Logs.LogTypes.debug,
					"available RNDIS device found, tell listener now");
			scanDevicesListener.onRndisDeviceFound(paramString1, paramString2);
		} else {
			Logs.log(Logs.LogTypes.debug,
					"available RNDIS device found, but no listener is set");
		}
	}

	public void sendCommand(Command paramCommand) throws Exception {
		...
		synchronized (socket) {
			if (!socket.isClosed()) {
				...
				Logs.log(Logs.LogTypes.debug,
						" - Sending - Command send successfully. type: "
								+ paramCommand.getPayload());
			} else {
				Logs.log(Logs.LogTypes.exception,
						"Socket was closed, retrying the connection. Status - "
								+ getState().toString());
			}
		}
	}
	...
	public static abstract interface ScanDevicesListener {
		public abstract void onBluetoothDeviceFound(String paramString,
				BluetoothDevice paramBluetoothDevice, boolean paramBoolean1,
				boolean paramBoolean2);
		...
		public abstract void onApDeviceFound(String paramString1,
				String paramString2);

		public abstract void onRndisDeviceFound(String paramString1,
				String paramString2);
	}

	public static abstract interface BleReceivedDataListener {
		public abstract void onBleDataReceived(ReceivedData paramReceivedData,
				ErrorObject paramErrorObject) throws DeviceException;
	}

	public static abstract interface WifiReceivedDataListener {
		public abstract void onResponseReceived(ReceivedData paramReceivedData,
				ErrorObject paramErrorObject);

		public abstract void onEventDataReceived(ReceivedData paramReceivedData);

		public abstract void onLiveImageDataReceived(
				ReceivedData paramReceivedData) throws DeviceException;
	}
