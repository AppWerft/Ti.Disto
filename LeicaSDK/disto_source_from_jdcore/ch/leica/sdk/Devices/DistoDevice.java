  ...
  extends Device
{
  ...
  public DistoDevice(Context paramContext, String paramString1, String paramString2, Types.ConnectionType paramConnectionType)
  {
  ...
  public DistoDevice(Context paramContext, String paramString1, String paramString2)
  {
  ...
  public void onConnected(BaseConnectionManager paramBaseConnectionManager)
  {
    ...
    responseHandler.postDelayed(new Runnable()
    {
      public void run()
      {
        ...
        Types.ConnectionType localConnectionType = getConnectionType();
        if ((localConnectionType == Types.ConnectionType.wifiAP) || (localConnectionType == Types.ConnectionType.rndis)) {
          str = deviceIP;
        }
          ...
          if (!getConnectionType().equals(Types.ConnectionType.rndis))
          {
            ...
            if (i == 1)
            {
              ...
              if (localConnectionType == Types.ConnectionType.wifiHotspot)
              {
                ...
                {
                  Logs.log(Logs.LogTypes.debug, "wifiName does not equal devicename. " + str + " vs " + getDeviceName() + " will call disconnect()");
                  disconnect();
                }
              }
              else if (localConnectionType == Types.ConnectionType.wifiAP)
              {
                WifiManager localWifiManager = (WifiManager)paramAnonymousContext.getApplicationContext().getSystemService("wifi");
                WaitAmoment localWaitAmoment = new WaitAmoment();
                localWaitAmoment.waitAmoment(1000L);
                if (!localWifiManager.isWifiEnabled())
                {
                  Logs.log(Logs.LogTypes.debug, "wifiName is off");
                  disconnect();
                }
              }
            }
        ...
        public void run()
        {
          ...
          if (getConnectionType() == Types.ConnectionType.wifiHotspot)
          {
            ...
            if (str == null)
            {
              ...
              ErrorObject.sendErrorIncorrectSSID(c, DistoDevice.this);
              return;
            }
            if (!str.equalsIgnoreCase(getDeviceName()))
            {
              Logs.log(Logs.LogTypes.codeerror, "wifiName does not equal devicename. " + str + " vs " + getDeviceName());
              ErrorObject.sendErrorIncorrectSSID(c, DistoDevice.this);
              return;
            }
          }
