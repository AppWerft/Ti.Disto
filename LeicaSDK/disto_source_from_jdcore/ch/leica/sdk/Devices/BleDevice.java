...
public class BleDevice extends Device {
	...
	public BleDevice(Context paramContext, String paramString,
			BluetoothDevice paramBluetoothDevice, boolean paramBoolean) {
		...
		Logs.log(Logs.LogTypes.debug, "hasDistoServiceBeforeConnection: "
				+ hasDistoServiceBeforeConnection + " deviceID: " + deviceID);
		init();
	}
	...
	protected void assignConnectionManager() {
		...
		((BleConnectionManager) connectionManager)
				.setNoPairingNeeded(hasDistoServiceBeforeConnection);
	}
	...
	public List<BleCharacteristic> getAllCharacteristics() {
		return ((BleConnectionManager) connectionManager)
	...
	protected void setConnectionParameters() {
		connectionManager
				.setConnectionParameters(new Object[] { bluetoothDevice });
	}
	...
	protected void handleDataParsing(ReceivedData paramReceivedData,
			ErrorObject paramErrorObject) {
		synchronized (i) {
	...
	public void registerReceivers() {
		if (e == null) {
			e = new BroadcastReceiver() {
				public void onReceive(Context paramAnonymousContext,
					...
					if (str.equals("android.bluetooth.adapter.action.STATE_CHANGED")) {
						int i = paramAnonymousIntent.getIntExtra(
								"android.bluetooth.adapter.extra.STATE",
								Integer.MIN_VALUE);
						switch (i) {
						case 10:
						case 13:
							Logs.log(Logs.LogTypes.debug, "action STATE OFF: "
									+ str);
							disconnect();
							ErrorObject.sendErrorBluetoothIsOff(c,
									BleDevice.this);
							break;
						case 12:
							Logs.log(Logs.LogTypes.debug, "action STATE ON: "
									+ str);
							break;
						case 11:
							Logs.log(Logs.LogTypes.debug,
									"action STATE TURNING ON: " + str);
							break;
						default:
			...
			Logs.log(Logs.LogTypes.verbose,
					"bluetoothAdapterChangedReceiver start registering");
			IntentFilter localIntentFilter = new IntentFilter(
					"android.bluetooth.adapter.action.STATE_CHANGED");
			context.registerReceiver(e, localIntentFilter);
			Logs.log(Logs.LogTypes.debug,
					"bluetoothAdapterChangedReceiver registered");
		}
		if ((Build.VERSION.SDK_INT < 21) && (f == null)) {
			f = new BroadcastReceiver() {
				public void onReceive(Context paramAnonymousContext,
						Intent paramAnonymousIntent) {
					String str1 = paramAnonymousIntent
							.getStringExtra("deviceName");
					String str2 = paramAnonymousIntent
							.getStringExtra("deviceAddress");
					Logs.log(Logs.LogTypes.debug, "deviceName: " + str1
							+ ", deviceAddress: " + str2);
				}
			};
			Logs.log(Logs.LogTypes.verbose,
					"deviceBondedReceiver start registering");
			LocalBroadcastManager.getInstance(context).registerReceiver(f,
					new IntentFilter("DEVICE_BONDED"));
		...
		if (i == 1) {
			...
			connectionHandler.post(new Runnable() {
				public void run() {
					...
					registerReceivers();
					((BleConnectionManager) connectionManager)
							.setDiscoverServicesDelay(getDiscoverServicesDelay());
	...
	public void disconnect() {
		if ((getConnectionState() == Device.ConnectionState.connected)
				&& (!((BleConnectionManager) connectionManager)
						.isNoPairingNeeded())
				&& (!deviceType.equals(Types.DeviceType.Yeti))) {
			Logs.log(Logs.LogTypes.debug,
					"device needs pairing. so disconnect with unpair() for "
							+ deviceID);
			((BleConnectionManager) connectionManager).unBond();
		}
		if (((BleConnectionManager) connectionManager).getGattStatus() == 133) {
			if (c != null) {
				c.onError(
						new ErrorObject(
								2112,
								"Received a Gatt Error 133. Unfortunately this is unavoidable. The only solution is retrying to establish the connection."),
						this);
			}
		} else if ((((BleConnectionManager) connectionManager).getGattStatus() == 62)
				&& (c != null)) {
			c.onError(
					new ErrorObject(
							2114,
							"Received a Gatt Error 62. This means establishing connection failed. In some cases, this is due to a bad signal strength received by the bluetooth antenna. This can be resolved by bringing the phone/tablet nearer to the device and just try again."),
					this);
		}
		super.disconnect();
	}
	...
	public void onBleDataReceived(ReceivedData paramReceivedData,
			ErrorObject paramErrorObject) {
		if ((dataPacket != null)
				&& ((dataPacket.dataId.equals("DS_MODEL_NAME")) || (dataPacket.dataId
						.equals("DI_MODEL_NUMBER")))
				&& (modelName.equals("") == true)) {
			modelName = a(paramReceivedData);
			c();
		}
		if (i == null) {
			Logs.log(Logs.LogTypes.debug,
					"currentResponse is null, data is an event.");
			if (paramErrorObject != null) {
				...
				Logs.log(Logs.LogTypes.verbose,
						"call this.receivedDataListener.onAsyncDataReceived()");
				a.onAsyncDataReceived(paramReceivedData);
			}
			return;
		}
		if (bleResponseHandler == null) {
			bleResponseThread = new HandlerThread("Device_BleResponseThread_"
		...
		synchronized (j) {
			if (!j.booleanValue()) {
				...
				Logs.log(Logs.LogTypes.debug, "waitingForBleResponsesTime: "
	...
	private String a(ReceivedData paramReceivedData) {
		...
		if (dataPacket != null) {
			try {
				...
				Logs.log(Logs.LogTypes.exception, "Error Getting the Model.",
						localException);
			}
		}
		return str1;
	...
	private void c() {
		...
		if (modelName != null) {
			if ("D510".equals(modelName) == true) {
				...
				k = new String[] { Types.Commands.Custom.name(),
						Types.Commands.Distance.name(),
						Types.Commands.LaserOn.name(),
						Types.Commands.LaserOff.name(),
						Types.Commands.StartTracking.name(),
						Types.Commands.StopTracking.name() };
			}
		} else {
			k = new String[0];
		}
	...
	public boolean isBleOperationInProgress() {
		return ((BleConnectionManager) connectionManager)
				.isBleOperationInProgress();
	}

	public void readAllBleCharacteristics(
			Device.BTConnectionCallback paramBTConnectionCallback) {
		((BleConnectionManager) connectionManager)
				.readAllCharacteristics(paramBTConnectionCallback);
	}
}
