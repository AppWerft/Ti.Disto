...
public abstract class Device implements ErrorListener,
		BaseConnectionManager.BleReceivedDataListener,
		BaseConnectionManager.ConnectionListener,
		BaseConnectionManager.WifiReceivedDataListener, Serializable {
	...
	public Device(Context paramContext, Types.ConnectionType paramConnectionType) {
		...
		Logs.log(Logs.LogTypes.debug, "Created DEVICE with deviceID: "
				+ deviceID);
	}
	...
	public abstract void onConnected(
			BaseConnectionManager paramBaseConnectionManager);
	...
	public void onBleDataReceived(ReceivedData paramReceivedData,
			ErrorObject paramErrorObject) throws DeviceException {
	...
	public void onError(ErrorObject paramErrorObject, Device paramDevice) {
		if (c != null) {
			if (paramErrorObject.getErrorCode() == 2802) {
				paramErrorObject.setErrorMessage(paramErrorObject
						.getErrorMessage()
						+ "The SSID should be: "
						+ getDeviceName());
			}
	...
	public void onResponseReceived(ReceivedData paramReceivedData,
	...
	public void disconnect() {
		...
		Logs.log(Logs.LogTypes.debug,
				"Disconnected successfully from the device. ");
	}
	...
	public synchronized Response processCommand(final Command paramCommand,
			long paramLong) throws DeviceException {
		Response localResponse;
		try {
			Logs.log(Logs.LogTypes.debug,
					"Called with type: " + paramCommand.getCommandValue()
							+ "Value: " + paramCommand.getPayload());
			if (connectionState != ConnectionState.connected) {
				Logs.log(Logs.LogTypes.debug, "State is NOT CONNECTED");
				throw new DeviceException("Device is not connected");
			}
			synchronized (b) {
				if ((b == DeviceState.update)
						&& (!paramCommand.isUpdateCommand())) {
					Logs.log(Logs.LogTypes.debug,
							"device is in update process.");
			...
			if (i == null) {
				throw new DeviceException(
						"Internal error. Could not send command (null command).");
			}
			boolean bool = true;
				...
				public void run() {
					try {
						Logs.log(Logs.LogTypes.verbose,
								"Start of handler thread. Now send command with connection manager");
						connectionManager.sendCommand(paramCommand);
					} catch (Exception localException) {
						Logs.log(Logs.LogTypes.exception,
								"send command failed. e.message: "
			...
			Logs.log(Logs.LogTypes.exception, "Error processing the message. "
					+ localException.getMessage(), localException);
			ErrorObject localErrorObject = new ErrorObject(3508,
					localException.getMessage());
	...
	public void readAllBleCharacteristics(
			BTConnectionCallback paramBTConnectionCallback)
			throws DeviceException {
		throw new DeviceException("Method not implemented for this device");
	}
	...
	public void sendCommandMotorPositionAbsolute(double paramDouble1,
			double paramDouble2, boolean paramBoolean) throws DeviceException {
		throw new DeviceException("Method not implemented for this device");
	}

	public void sendCommandMotorPositionRelative(double paramDouble1,
			double paramDouble2, boolean paramBoolean) throws DeviceException {
		throw new DeviceException("Method not implemented for this device");
	}

	public void sendCommandMoveMotorDown(int paramInt) throws DeviceException {
		throw new DeviceException("Method not implemented for this device");
	}

	public void sendCommandMoveMotorLeft(int paramInt) throws DeviceException {
		throw new DeviceException("Method not implemented for this device");
	}

	public void sendCommandMoveMotorRight(int paramInt) throws DeviceException {
		throw new DeviceException("Method not implemented for this device");
	}

	public void sendCommandMoveMotorUp(int paramInt) throws DeviceException {
		throw new DeviceException("Method not implemented for this device");
	}
	...
	public void setReceiveDataListener(
			ReceivedDataListener paramReceivedDataListener) {
	...
	public void unpairDevice() {
		Logs.log(Logs.LogTypes.codeerror,
				"Method not implemented for this device");
	}
	...
	public void unregisterReceivers() {
		try {
			...
			Logs.log(Logs.LogTypes.debug,
					"bluetoothAdapterChangedReceiver unregistered");
			if (Build.VERSION.SDK_INT < 21) {
				...
				Logs.log(Logs.LogTypes.debug,
						"deviceBondedReceiver unregistered");
			}
			if (d != null) {
			...
			Logs.log(Logs.LogTypes.exception,
					"Error Unregistering the Receiver", localException);
		}
	}

	public ResponseUpdate updateDeviceFirmwareWithFirmwareUpdate(
			FirmwareUpdate paramFirmwareUpdate,
			UpdateDeviceListener paramUpdateDeviceListener)
			throws DeviceException {
		throw new DeviceException("Method not implemented for this device");
	}
	...
	void b() {
		...
		commandThread = new HandlerThread("Device_CommandThread_"
		...
		responseThread = new HandlerThread("Device__ResponseThread_"
		...
		eventThread = new HandlerThread("Device__EventThread_"
				+ System.currentTimeMillis());
		eventThread.start();
		...
		connectionThread = new HandlerThread("Device__ConnectionThread_"
				+ System.currentTimeMillis(), 10);
	...
	private class a extends TimerTask {
		...
		public void run() {
			if (i == null) {
				Logs.log(Logs.LogTypes.codeerror,
						"current response is null. this should never happen.");
				return;
			}
			synchronized (i) {
				i.setError(new ErrorObject(3502,
						"No response received within given timeframe."));
				synchronized (h) {
					h = Boolean.valueOf(false);
				}
				i.setWaitingForData(Boolean.valueOf(false));
	...
	public static abstract interface UpdateDeviceListener {
		public abstract void onFirmwareUpdateStarted(String paramString1,
				String paramString2);
	...
	public static abstract interface ConnectionListener {
		public abstract void onConnectionStateChanged(Device paramDevice,
				Device.ConnectionState paramConnectionState);
	}
