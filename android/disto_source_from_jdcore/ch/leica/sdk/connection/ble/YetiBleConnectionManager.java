...
public class YetiBleConnectionManager
  extends BleConnectionManager
{
  public YetiBleConnectionManager(BluetoothManager paramBluetoothManager, Context paramContext)
  {
    ...
    if (retryPairing < RETRYPAIRINGLIMIT)
    {
      synchronized (this)
      {
        ...
        if (localBluetoothDevice != null)
        {
          if (!noPairingNeeded)
          {
            Logs.log(Logs.LogTypes.debug, "check if pairing needed!");
            if (Build.VERSION.SDK_INT >= 21)
            {
              if (localBluetoothDevice.getBondState() != 12)
              {
                Logs.log(Logs.LogTypes.debug, "bluetoothDevice.getBondState() != BluetoothDevice.BOND_BONDED ! so pairing needed!");
                if (retryPairing == 0)
                {
                  bool = pairAndBond();
                }
